...
for callsign in iff_data[3].callsign.unique():
    ...
    flightTakenOff = iff.check_if_flight_has_departed(iff_data[3],callsign,natsSim,departureAirport)
    ...
    if not flightTakenOff:
        ...
        txt1 = iff.create_gate_to_runway_from_iff(trackData,natsSim,departureAirport)
        txt2 = iff.create_gate_to_runway_trx_using_nats(trackData,natsSim,departureAirport)
    else:
